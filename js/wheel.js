...
(function () {
  ...
  function makeWheel(R, size) {
    ...
    // фоновые окружности
    s += `<circle class="draw" pathLength="1" cx="${cx}" cy="${cy}" r="${rOut}" fill="var(--w-bg)" stroke="var(--w-line)" stroke-width="1.5"/>`;
    s += `<circle class="draw" pathLength="1" cx="${cx}" cy="${cy}" r="${rSign}" fill="none" stroke="var(--w-line)" stroke-width="1"/>`;
    s += `<circle class="draw" pathLength="1" cx="${cx}" cy="${cy}" r="${rHouse}" fill="none" stroke="var(--w-line)" stroke-width="1" stroke-opacity="0.7"/>`;
    s += `<circle class="draw" pathLength="1" cx="${cx}" cy="${cy}" r="${rHub}" fill="var(--w-hub)" stroke="var(--w-line)" stroke-width="1" stroke-opacity="0.6"/>`;

    // секторы знаков + глифы + деления
    for (let i = 0; i < 12; i++) {
      const l0 = i * 30;
      s += `<path class="fade" d="${ring(l0, l0 + 30, rSign, rOut)}" fill="var(${ELEM_VAR[i % 4]})" fill-opacity="0.30" stroke="none"/>`;
      s += `<line class="draw" pathLength="1" x1="${P(l0, rSign).split(",")[0]}" y1="${P(l0, rSign).split(",")[1]}"
             x2="${P(l0, rOut).split(",")[0]}" y2="${P(l0, rOut).split(",")[1]}"
             stroke="var(--w-line)" stroke-width="1"/>`;
      const [gx, gy] = pt(l0 + 15, (rSign + rOut) / 2);
      s += `<text class="fade" x="${gx.toFixed(1)}" y="${(gy + 7).toFixed(1)}" text-anchor="middle"
      ...
      for (let dgr = 0; dgr < 30; dgr += 5) {
        ...
        s += `<line class="fade" x1="${x1.toFixed(1)}" y1="${y1.toFixed(1)}" x2="${x2.toFixed(1)}" y2="${y2.toFixed(1)}"
    ...
    for (let h = 0; h < 12; h++) {
      ...
      s += `<text class="fade" x="${hx.toFixed(1)}" y="${(hy + 4).toFixed(1)}" text-anchor="middle"
      ...
      s += `<line class="draw" pathLength="1" x1="${sx1.toFixed(1)}" y1="${sy1.toFixed(1)}" x2="${sx2.toFixed(1)}" y2="${sy2.toFixed(1)}"
    ...
    function axis(lon, label, strong) {
      ...
      s += `<line class="${strong ? "draw" : "fade"}" ${strong ? 'pathLength="1"' : ""} x1="${x1.toFixed(1)}" y1="${y1.toFixed(1)}" x2="${x2.toFixed(1)}" y2="${y2.toFixed(1)}"
             stroke="var(--w-axis)" stroke-width="${strong ? 2 : 1.2}" stroke-dasharray="${strong ? "none" : "4 4"}"/>`;
      const [lx, ly] = pt(lon, rOut + size*0.022);
      s += `<text class="fade" x="${lx.toFixed(1)}" y="${(ly + 4).toFixed(1)}" text-anchor="middle"
    ...
    (R.aspects || []).forEach((a) => {
      ...
      s += `<line class="draw" pathLength="1" x1="${x1.toFixed(1)}" y1="${y1.toFixed(1)}" x2="${x2.toFixed(1)}" y2="${y2.toFixed(1)}"
    ...
    placed.forEach(({ p, r }) => {
      ...
      s += `<line class="fade" x1="${tx1.toFixed(1)}" y1="${ty1.toFixed(1)}" x2="${tx2.toFixed(1)}" y2="${ty2.toFixed(1)}"
             stroke="var(--w-line)" stroke-width="0.8" stroke-opacity="0.6"/>`;
      const [px, py] = pt(p.lon, r);
      s += `<circle class="fade" cx="${px.toFixed(1)}" cy="${py.toFixed(1)}" r="${(size*0.026).toFixed(1)}"
             fill="var(--w-planet-bg)" stroke="var(--w-planet-stroke)" stroke-width="1"/>`;
      s += `<text class="fade" x="${px.toFixed(1)}" y="${(py + size*0.012).toFixed(1)}" text-anchor="middle"
      ...
      s += `<text class="fade" x="${dx.toFixed(1)}" y="${(dy + 3).toFixed(1)}" text-anchor="middle"
             font-size="${(size*0.016).toFixed(0)}" fill="var(--w-muted)">${deg}°${p.retro ? " ℞" : ""}</text>`;
    });

    return `<svg viewBox="0 0 ${size} ${size}" xmlns="http://www.w3.org/2000/svg" class="natal-wheel draw-svg">${s}</svg>`;
  }
